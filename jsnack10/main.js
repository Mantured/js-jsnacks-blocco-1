...
for (let i = 0; i < 10; i++) {
    randomUser =
        nomi[Math.floor(Math.random() * nomi.length)][0].toUpperCase() + nomi[Math.floor(Math.random() * nomi.length)].substring(1)
        + " " +
        cognomi[Math.floor(Math.random() * cognomi.length)][0].toUpperCase() + cognomi[Math.floor(Math.random() * cognomi.length)].substring(1);
    falseUsers.push(randomUser);
}
